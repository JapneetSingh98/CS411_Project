...
int main() {
...
#else
    string str[28] = {"-","a","b","bagel","balloon","bat","f","fox","g","h","ha","happy","house","l","m",
                      "n","o","p","q","r","s","t","u","v","w","x","y","z"};
    ...
    for (int i=1; i<n; i++) {
         ...
         for (int j = 0; j < result.size(); j++) {
             cout<<"\nResult : " << term << " " <<result[j].id << "   " << result[j].name << "\n";
         }
    }
#else
  while (true) {    
    ...
    for (int i = 0; i < result.size(); i++) {
        cout<<"\n " << term << " " << result[i].id << "   " << result[i].name << "\n";
    }
